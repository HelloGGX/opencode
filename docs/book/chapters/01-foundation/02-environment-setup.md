...
现在我们已经具备了完整的Monorepo开发环境。但在开始编写代码之前，我们需要先理解一个基础问题：当用户在终端输入 `opencode --version` 时，操作系统是如何找到并执行我们的程序的？

这个看似简单的问题，背后涉及操作系统的进程管理、环境变量、符号链接等多个底层机制。理解这些原理，对于构建一个健壮的 CLI 工具至关重要。

下一章，我们将深入探讨 CLI 触发的完整生命周期，从系统寻址到程序执行，逐步揭开命令行工具的底层机制。
