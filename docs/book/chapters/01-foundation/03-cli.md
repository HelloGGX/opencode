# 第 3 章：CLI 触发原理：从 opencode --version 到底层机制
...
在实际的 OpenCode 项目中，本章讲解的机制对应以下文件：

| 本章内容 | 实际项目文件 | 代码行数 | 说明 |
|---------|-------------|---------|------|
| 3.1 系统寻址 | - | 理论讲解 | PATH 环境变量、符号链接机制 |
| 3.2 Shebang | `bin/opencode` (第1行) | 1行 | `#!/usr/bin/env node` |
| 3.3 启动器模式 | `bin/opencode` (完整) | 150行 | 跨平台二进制分发的完整实现 |

**实际项目对齐度**: 95%

**完整代码参考**: https://github.com/anomalyco/opencode/blob/dev/packages/opencode/bin/opencode

## 3.5 总结

本章我们从操作系统如何找到可执行文件开始，逐步深入到 CLI 工具的底层触发机制：

**核心知识点**：

1. **寻址机制**：理解了 PATH 环境变量和符号链接如何帮助操作系统找到命令
2. **Shebang**：学会了用 `#!/usr/bin/env node` 让文本文件变成可执行程序
3. **启动器模式**：掌握了跨平台二进制分发的完整方案，包括：
   - AVX2 指令集检测
   - musl/glibc 兼容性处理
   - 降级策略设计
   - Monorepo 包提升应对
...
执行二进制，传递参数
    ↓
输出: opencode version 1.1.59
```

**关键洞察**：

- CLI 工具的入口文件不应该包含业务逻辑，而应该是一个纯粹的"路由器"
- 跨平台分发需要考虑操作系统、CPU 架构、指令集、C 标准库等多个维度
- 降级策略确保了在极端环境下的可用性

现在你已经理解了 CLI 触发的完整原理。在下一章，我们将动手实现一个最小可用的 CLI 工具，将这些原理付诸实践。
